...
namespace SoftRender.UnityApp
{
    static public class GeometricFactory
    {
                                        ...
                                        bool compute_normals, bool compute_tangent)
        {
            ...
            for (int y = 0; y < vertex_per_side; y++)
            {
                for (int x = 0; x < vertex_per_side; x++)
                {
                    ...
                    float h = Mathf.Perlin(uv * noise_scale) * max_height;
                                            ...
                                            Vector2 noise_scale, Vector2 noise_strength)
        {
            ...
            for (int i = 0; i < nVertex; i++)
            {
                float perlinNoise = Mathf.Perlin(new Vector2(uv[i].x * noise_scale.x, uv[i].y * noise_scale.y)) * (noise_strength.y - noise_strength.x) + noise_strength.x;
        ...
        public static Mesh BuildAxis(string meshName, Vector2 min_size, Vector2 max_size, float bottom_size, float top_size, int subdivs)
        {
            ...
            MeshTools.SetColor0(arrowX, new Color(1, 0, 0, 1));
            ...
            MeshTools.SetColor0(arrowY, new Color(0, 1, 0, 1));
            ...
            MeshTools.SetColor0(arrowZ, new Color(0, 0, 1, 1));
